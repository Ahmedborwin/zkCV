...
const ShowCV = ({ groupId }) => {
    ...
    const handleSelectCVs = async () => {
        ...
        const txHash = await chooseApplications(provider, zkCV, groupId, selectCVHashes, dispatch)
        ...
        <div className="border-b border-gray-300 pb-4 mb-4">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
                {ipfsContentList.map((content, index) => (
                    <div
                        key={index}
                        className="flex items-center gap-2 bg-blue-500 text-white p-2 rounded-lg hover:bg-blue-600 transition duration-150 ease-in-out w-full max-w-s"
                    >
                        <input
                            type="checkbox"
